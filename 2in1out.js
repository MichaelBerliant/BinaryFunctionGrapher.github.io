...
function validateInput() {
    ...
    try {
        ...
        const sandbox = (fn) => {
            let hasValidOutput = false; // Flag to track if a valid output is found

            // Iterate over geometry positions
            for (let i = 0; i < geometry.attributes.position.count; i++) {
                const x = geometry.attributes.position.getX(i);
                const y = geometry.attributes.position.getY(i);

                try {
                    const z = fn(x, y); // Evaluate the function at (x, y)

                    // Check if z is valid
                    if (isFinite(z)) {
                        hasValidOutput = true;
                        return; // Stop checking further points
                    }
                } catch (error) {
            ...
            // If no valid output is found, throw an error
            if (!hasValidOutput) {
